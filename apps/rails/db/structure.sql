...
CREATE TABLE public.selling_locations (
    ...
    kind character varying DEFAULT 'ponto'::character varying NOT NULL,
    position_updated_at timestamp(6) without time zone,
    discarded_at timestamp(6) without time zone,
    CONSTRAINT selling_locations_kind_check CHECK (((kind)::text = ANY (ARRAY[('ponto'::character varying)::text, ('circulando'::character varying)::text])))
);
...
--
-- Name: index_selling_locations_on_one_roaming_per_seller; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX index_selling_locations_on_one_roaming_per_seller ON public.selling_locations USING btree (seller_profile_id) WHERE ((kind)::text = 'circulando'::text);
